/**
 * Ported from web/src/components/graph-canvas.tsx, which stays where it is and keeps working.
 *
 * Two changes, both forced by the move. web/ is Tailwind and this app is not, so the utility
 * strings moved into memory-graph.css with the same values. And the first view is fitted to
 * the layout's extent, because the viewBox was sized for the five-turn fixture and a real
 * graph now settles about 1900x1140 units wide: cropping the middle of that into a column
 * would show a handful of nodes and nothing else. Palette, radii, edge encoding and the
 * drag / zoom / isolate gestures are untouched.
 */
...
export function GraphCanvas({
  ...
  /** Armed once per layout: a refit after every drag would yank the view out from under it. */
  ...
  /** Frame the whole layout, centred, through the same zoom behaviour the gestures drive. */
  const fitToExtent = useCallback(() => {
    const svg = svgRef.current;
    const g = gRef.current;
    const z = zoomRef.current;
    if (!svg || !g || !z) return;
    // getBBox reports the group's own coordinate space, so the current zoom is not baked in.
    const box = g.getBBox();
    if (!box.width || !box.height) return;
    const vb = svg.viewBox.baseVal;
    const rect = svg.getBoundingClientRect();
    if (!rect.width || !rect.height) return;
    // preserveAspectRatio defaults to meet, so one axis shows more user space than the
    // viewBox names. Fitting against the viewBox alone would waste that axis.
    const perUnit = Math.min(rect.width / vb.width, rect.height / vb.height);
    const k = Math.min(rect.width / perUnit / box.width, rect.height / perUnit / box.height) * 0.92;
    const cx = box.x + box.width / 2;
    const cy = box.y + box.height / 2;
    select(svg).call(z.transform, zoomIdentity.translate(-k * cx, -k * cy).scale(k));
  }, []);
  ...
  const resetView = () => {
    ...
    // Re-armed rather than snapped to 1:1: identity is the crop this fit exists to avoid.
    fitPendingRef.current = true;
  ...
  return (
    <div className="mnemex-graph-canvas">
      <svg
        ref={svgRef}
      ...
      <div className="mnemex-graph-legend">
        {Object.entries(PALETTE).map(([label, color]) => (
          <span key={label}>
            <span className="mnemex-graph-dot" style={{ background: color }} />
            {label}
          </span>
        ))}
        <span>
          <span className="mnemex-graph-rule" style={{ background: DISSENT }} />disagrees
        </span>
        <span>
          <span className="mnemex-graph-rule-dashed" />supersedes
        </span>
      </div>

      <button className="mnemex-graph-reset" onClick={resetView}>
        reset layout
      </button>
      <p className="mnemex-graph-hint">
