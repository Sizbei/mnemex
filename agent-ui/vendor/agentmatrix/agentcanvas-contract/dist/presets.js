import { completeAgentCanvasExperience, defaultAgentCanvasExperience, } from "./defaults.js";
export const workspaceSafeCapabilities = {
    provider: false,
    liveRun: false,
    git: false,
    gitMutation: false,
...
export const experiencePresetGroups = [
    {
        id: "layout",
        label: "Layout",
        options: [
            option("sidebar-visible", "Show sidebar", "Show session navigation beside the conversation.", "Regions"),
            option("output-visible", "Show output panel", "Show generated artifacts in a bounded output surface.", "Regions"),
            option("git-visible", "Git status", "Show review-only Git status in the output region.", "Regions", "git"),
            option("debug-visible", "Debug dock", "Show developer diagnostics below the main surface.", "Regions", "debug"),
        ],
    },
    {
        id: "conversation",
        label: "Conversation",
        options: [
            option("writing-smooth", "Smooth stream", "Reveal assistant text as a steady stream.", "Writing"),
            option("writing-typewriter", "Typewriter", "Use a measured character rhythm.", "Writing"),
            option("writing-chunked", "Chunked", "Reveal output in readable phrase chunks.", "Writing"),
            option("speaker-labels", "Speaker labels", "Show user and agent labels on turns.", "Message chrome"),
            option("message-actions", "Message actions", "Expose copy, regenerate, and edit controls.", "Recovery"),
        ],
    },
    {
        id: "sidebar",
        label: "Sidebar",
        options: [
            option("sidebar-new-button", "New chat button", "Show a new-session action.", "Content"),
            option("sidebar-search", "Search box", "Show session search.", "Content"),
            option("sidebar-grouping", "Group by date", "Group sessions under time headings.", "Content"),
            option("sidebar-footer", "Footer note", "Show the muted sidebar status line.", "Content"),
        ],
    },
    {
        id: "ux-effects",
        ...
        options: [
            ...
            option("thinking-pulse", "Pulse scan", "A compact active-reasoning scan.", "Motion"),
            option("thinking-terminal", "Terminal cursor", "Console-style progress.", "Motion"),
            option("thinking-minimal", "Minimal line", "A quiet status line.", "Motion"),
            option("thinking-shimmer", "Shimmer text", "A light sweep across status text.", "Motion"),
            option("thinking-bars", "Wave bars", "Small equalizer bars.", "Motion"),
            option("thinking-orbit", "Orbit dot", "A compact orbit loader.", "Motion"),
            option("summary-first", "Summary before details", "Show a safe public summary before tool and code details.", "Disclosure"),
            option("reasoning-auto-collapse", "Auto collapse", "Keep completed reasoning compact.", "Disclosure"),
            option("reasoning-expanded", "Expanded by default", "Open safe reasoning summaries by default.", "Disclosure"),
            option("reasoning-status-only", "Status only", "Show only public execution state.", "Disclosure"),
            option("reasoning-public-summary", "Public summary", "Show a safe summary, never hidden chain-of-thought.", "Disclosure"),
        ],
    },
    {
        id: "tool-calls",
        label: "Tool Calls",
        options: [
            option("command-cards", "Command cards", "Expandable cards with lifecycle and safe detail.", "Layout"),
            option("compact-chips", "Compact chips", "Dense inline tool status.", "Layout"),
            option("timeline-rail", "Timeline rail", "Sequential tool activity rail.", "Layout"),
            option("terminal-log", "Terminal drawer", "Command output in a bounded drawer.", "Layout", "liveRun"),
            option("tool-detail-full", "Input and output", "Show safe arguments and visible results.", "Details"),
            option("tool-detail-output-only", "Output only", "Hide tool arguments.", "Details"),
            option("tool-detail-summary", "Summary only", "Keep only a compact preview.", "Details"),
            option("tool-progress-icon", "Status icon", "Use lifecycle icons.", "Lifecycle"),
            option("tool-progress-bar", "Progress bar", "Add a compact progress rail.", "Lifecycle"),
            option("tool-approval-inline", "Inline approval", "Show approval actions in the tool card.", "Lifecycle"),
            option("tool-approval-hidden", "External approval", "Keep approval actions outside tool detail.", "Lifecycle"),
        ],
    },
    {
        id: "blocks",
        label: "Blocks",
        options: [
            option("code-diff", "Code diffs", "Prefer a diff renderer for patch artifacts.", "Content"),
            option("tool-log-tail", "Log tails", "Show compact tails for long tool logs.", "Content"),
            option("error-collapse", "Error collapse", "Keep debug detail out of the primary transcript.", "Failure states"),
        ],
    },
    {
        id: "composer",
        label: "Composer",
        options: [
            option("upload", "Upload", "Allow file attachment affordances.", "Inputs"),
            option("mic", "Mic", "Add optional voice input.", "Inputs"),
            option("budget", "Thinking budget", "Show reasoning budget controls.", "Run controls"),
            option("model-tools", "Model and tools", "Show model and tool controls without credentials.", "Run controls", "provider"),
            option("prompt-shortcuts", "Prompt chips", "Offer reusable prompt shortcuts.", "Shortcuts"),
        ],
    },
    {
        id: "output",
        label: "Output",
        options: [
            option("output-source-artifact", "Latest artifact", "Use the newest artifact as output.", "Source"),
            option("output-source-console", "Console logs", "Use visible run and tool output.", "Source", "liveRun"),
            option("surface-right-panel", "Right panel", "Keep output in a bounded side panel.", "Surface"),
            option("surface-overlay", "Overlay", "Open output as a focused overlay.", "Surface"),
        ],
    },
    {
        id: "render",
        label: "Render",
        options: [
            option("renderer-auto", "Auto renderer", "Choose from artifact metadata.", "Artifact renderer"),
            option("renderer-code", "Code", "Render source code.", "Artifact renderer"),
            option("renderer-diff", "Diff", "Render patches with stable gutters.", "Artifact renderer"),
            option("renderer-markdown", "Markdown", "Render structured documents.", "Artifact renderer"),
            option("renderer-preview", "HTML / app preview", "Render visual output.", "Artifact renderer"),
            option("renderer-data", "Data / form", "Render structured data.", "Artifact renderer"),
        ],
    },
    {
        id: "theme",
        label: "Theme",
        options: [
            option("console-light", "Console light", "Neutral light product surface.", "Preset"),
            option("graphite", "Graphite mono", "Dark operational surface.", "Preset"),
            option("oxide", "Oxide workbench", "Warm surface with grounded actions.", "Preset"),
            option("studio-neutral", "Studio neutral", "Cool neutral product surface.", "Preset"),
            option("paper-trail", "Paper trail", "Editorial paper-toned surface.", "Preset"),
            option("terminal-green", "Terminal green", "Dark console surface.", "Preset"),
        ],
    },
];
export function experiencePresetGroupsForCapabilities(capabilities = workspaceSafeCapabilities) {
    const resolved = { ...workspaceSafeCapabilities, ...capabilities };
    return experiencePresetGroups
        .map((group) => ({
        ...group,
        options: group.options.filter((entry) => !entry.capability || resolved[entry.capability]),
    }))
        .filter((group) => group.options.length > 0);
}
export function experiencePresetGroupsForExperience(input, capabilities = workspaceSafeCapabilities) {
    const value = completeAgentCanvasExperience(input);
    return experiencePresetGroupsForCapabilities(capabilities)
        .map((group) => ({
        ...group,
        options: group.options.filter((entry) => isPresetOptionSupportedByExperience(value, entry.id)),
    }))
        .filter((group) => group.options.length > 0);
}
export function applyExperiencePresetOption(input, optionId) {
    const value = completeAgentCanvasExperience(input);
    const setMotion = (key, next) => ({
        ...value,
        theme: { ...value.theme, motion: { ...value.theme.motion, [key]: next } },
    });
    const setSlot = (component) => {
        const existing = value.layout.slots.filter((slot) => slot.component === component);
        const fallback = defaultAgentCanvasExperience.layout.slots.find((slot) => slot.component === component);
        ...
        return {
            ...
            layout: {
                ...value.layout,
                slots: existing.length > 0
                    ? value.layout.slots.map((slot) => slot.component === component
    ...
    switch (optionId) {
        ...
        case "code-diff": {
            const enabled = value.blocks.codeDiff && value.output.artifactRenderer === "diff";
            if (!enabled)
                assertRendererSupported(value, "diff");
            return {
                ...value,
        ...
        case "message-actions": {
            const enabled = !(value.conversation.messageActions.copy &&
        ...
        case "model-tools": {
            const enabled = !(value.composer.modelSwitcher && value.composer.toolToggle);
            return {
                ...value,
                composer: {
                    ...value.composer,
        ...
        case "terminal-green":
            return {
                ...value,
                theme: { ...value.theme, preset: optionId },
            };
        default:
            throw new Error(`Unsupported AgentCanvas preset option: ${optionId}`);
    }
}
export function isExperiencePresetOptionActive(input, optionId) {
    const value = completeAgentCanvasExperience(input);
    const slotEnabled = (component) => value.layout.slots.some((slot) => slot.component === component && slot.enabled);
    const direct = {
        ...
        "message-actions": value.conversation.messageActions.copy &&
            value.conversation.messageActions.regenerate &&
            value.conversation.messageActions.edit,
        ...
        "code-diff": value.blocks.codeDiff && value.output.artifactRenderer === "diff",
    ...
    if (Object.hasOwn(direct, optionId))
        return direct[optionId];
    const writing = {
...
export function toggleExperiencePresetOption(input, optionId) {
    ...
    if (["summary-first", "reasoning-auto-collapse", "reasoning-expanded"].includes(optionId))
        return {
            ...value,
...
function option(id, label, description, section, capability) {
    return { id, label, description, section, capability };
}
function withReasoningMotion(value, reasoning, show, collapse) {
    return {
        ...value,
...
function withToolLayout(value, toolCall, detail) {
    return {
        ...value,
...
function withOutputSurface(value, surface) {
    return {
        ...value,
        layout: {
            ...value.layout,
            slots: value.layout.slots.map((slot) => slot.component === "OutputFrame" ? { ...slot, region: surface } : slot),
        },
        output: { ...value.output, surface },
    };
}
function withRenderer(value, artifactRenderer) {
...
function isPresetOptionSupportedByExperience(value, optionId) {
    const renderer = rendererRequiredByPresetOption(optionId);
    return (renderer === undefined ||
        value.output.supportedArtifactRenderers.includes(renderer));
}
function rendererRequiredByPresetOption(optionId) {
    if (optionId === "code-diff")
        return "diff";
    const renderer = optionId.startsWith("renderer-")
        ? optionId.slice("renderer-".length)
        : "";
...
function assertRendererSupported(value, renderer) {
    if (!value.output.supportedArtifactRenderers.includes(renderer)) {
        throw new Error(`Unsupported AgentCanvas artifact renderer for this Experience: ${renderer}`);
    }
}
